...
Crie uma aplicação web contendo um campo para entrada de texto (input text) e dois botões (buttons), sendo um botão com a descrição "Adicionar" e outro com a descrição "Ordenar", conforme imagem abaixo:
...
function ordena(tipo){
    ...
    var objetos = document.getElementById('texto').value
    ...
    if (tipo=='adiciona'){
        ...
        }else if (objeto.indexOf(objetos)!==-1){
            alert('já foi adicionado')
        }  else{

            var auxiliar=[]

            auxiliar.push(document.getElementById('texto').value)

            objeto.push(auxiliar[0])
